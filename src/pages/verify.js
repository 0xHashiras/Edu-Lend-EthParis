...
export default function Page() {
  ...
  return (
    <InnerPageContainer title="Verify with Next.ID">
      ...
      <button className="btn btn-primary" onClick={() => setOption('twitter')}>Twitter</button> OR
      <button className="btn btn-primary" onClick={() => setOption('github')}>Github</button> <br />

      <button className="btn btn-primary mt-8" onClick={() => {
        setOption('ma')}}>Make Assetation</button> <br />
        <Link href={'acknowledgement'}>
        <button className="btn btn-primary mt-8" >Make Acknowledgement</button>
        </Link> <br />
        <Link href={'submissions'}>
        <button className="btn btn-primary mt-8" >My Submissions</button>
        </Link> <br />
        <Link href={'organization-verify'}>
        <button className="btn btn-primary mt-8" >Organization Verify Attestation</button>
        </Link> <br />
        <Link href={'organization-complete'}>
        <button className="btn btn-primary mt-8" >Organization Complete Attestation</button>
        </Link>
        ...
        <button className="btn btn-primary mt-8" type="submit">Submit Assetation</button>
          </form>
        ...
        <button className="btn btn-primary mt-8" type="button" onClick={getConfirmationAttestationsForUIDs}>Search Assetation</button>
          </>
          }
      <form onSubmit={verify}>
        {option === 'twitter' ? <div>
          ...
          <button className="btn btn-primary mt-8" onClick={getPublickey} style={{ marginRight: '5px' }}>get Key</button>
          ...
          option === 'github' ? <div>
            ...
            <input type="text" id="Public_key" value={public_key} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Enter public key" required onChange={(e) => setPublic_key(e.target.value)} />
            <button className="btn btn-primary mt-8" onClick={getPublickey} style={{ marginRight: '5px' }}>get Key</button>
            <button className="btn btn-primary mt-8" type="submit">Submit</button> </div> : <></>
        }
      </form>
      { payload != "" && (
        <div>
          {/* Content for the first div when showDiv is true */}
          <label htmlFor="Payload" className="block  text-sm font-medium dark:text-white">Payload = {payload} (without @)</label>
          <button className="btn btn-primary mt-8" onClick={sign_payload} style={{ marginRight: '5px' }}> Sign payload</button>
        </div>
      ) }
      { final_content != ""  &&
      (
        <div>
          {/* Content for the first div when showDiv is true */}
          <label htmlFor="Twitter postcontent" className="block  text-sm font-medium dark:text-white"> Post this on twitter = {final_content} </label>
          <button className="btn btn-primary mt-8" onClick={changeStatusIDbool} style={{ marginRight: '5px' }}> Post on twitter</button>
        </div>
      ) }
      {
        showStatusInput && (
          <div>
          <label htmlFor="status id input " className="block  text-sm font-medium dark:text-white"> Enter twitter post status ID (without @)</label> 
          ...
          <button className="btn btn-primary mt-8" onClick={verifypost} style={{ marginRight: '5px' }}> Verify account</button>
        </div>
      )}
    </InnerPageContainer>
  )
